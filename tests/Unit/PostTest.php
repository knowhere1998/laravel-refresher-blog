	...
	public function testGettingOnlyLastMonthPosts() {
		...
		factory(Post::class, 10)
			...
			->each(function($post) use ($faker) {
				$post->created_at = $faker->dateTimeBetween(Carbon::now()->subMonths(3), Carbon::now()->subMonths(2));
				$post->save();
			});
		// Newer Posts
		factory(Post::class, 3)
			->create()
			->each(function($post) use ($faker) {
				$post->created_at = $faker->dateTimeBetween(Carbon::now()->subWeeks(3), Carbon::now()->subWeeks(1));
				$post->save();
			});
		$isDuringLastMonth = true;
		foreach (Post::lastMonth($limit = 5)->get() as $post) {
			$isDuringLastMonth = $post->created_at->between(Carbon::now()->subMonth(), Carbon::now());
		}
